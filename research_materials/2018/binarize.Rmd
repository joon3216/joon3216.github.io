...
**ASIDE**: the function `change_form` can turn one form of data into another:

```{r eg2_2_7, eval = F}
change_form <- function(dat, from, to, 
                        response, variable.names, value.name,
                        responses, variable.name) {
    # dat: a data with categorical features
    # from: a character; 'binary', 'binomial', or 'poisson'
    # 
    # e.g.
    # change_form(
    #     rich, 
    ...
    #     response = 'is_rich',
    #     variable.names = c('rich', 'not_rich')
    ...
    if (from == 'binary') {
        ...
        dat <- eval(parse(text = paste0(
            'dat[, ',
            '.(', variable.names[1] , ' = sum(', response, '), ',
            variable.names[2], ' = sum(!', response, ')), ',
        ...
        if (to == 'binomial') {
            ...
            stop('\"to\" must be either \"binomial\" or \"poisson\"')
        }
    } else if (from == 'binomial') {
        id_vars <- col_names[!(col_names %in% responses)]
        id_vars_collapsed <- paste0(id_vars, collapse =',')
        if (to == 'binary') {
            binarize_binom(dat, responses, variable.name)
        } else if (to == 'poisson') {
            melt(
                dat,
                id.vars = id_vars,
            ...
            stop('\"to\" must be either \"binary\" or \"poisson\"')
        }
    } else if (from == 'poisson') {
        if (to == 'binary') {
            return(binarize_pois(dat, response))
        } else if (to == 'binomial') {
            id_vars <- col_names[!(col_names %in% new_response)]
            id_vars_fmlr <- paste0(id_vars, collapse = ' + ')
            fmlr <- paste0(id_vars_fmlr, ' ~ ', new_response)
            return(dcast(
                dat, 
                as.formula(fmlr), 
                value.var = response
            ...
            stop('\"to\" must be either \"binary\" or \"binomial\"')
        }
    } else {
        stop(paste0(
            '\"from\" must be either', 
...
```{r eg3_2_2, warning = F}
CI_auc(dead ~ smoker + age, femsmoke_binary)
CI_auc(is_rich ~ gender + criminal_record, rich_big, R = 200)
CI_auc(r ~ aged + stage + grade + xray + acid, nodal, type = 'perc')
```
