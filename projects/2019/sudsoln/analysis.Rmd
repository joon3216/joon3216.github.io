...
```{python scatter1, fig.asp = .6, fig.align = 'center'}
...
```{python line1, fig.asp = .6, fig.align = 'center', eval = F}
...
Although I didn't check whether the model is appropriate, the regression line fits nicely in the scatterplot of `log(time)` vs. `log(trial + 1)`. It seems we don't need to implement a nonlinear model, and need to deal with a fanning effect only.

```{python scatter3, fig.asp = .6, fig.align = 'center', eval = F}
...
plt.ylabel('time (in sec)')
plt.title('Scatterplot of time vs. trial')
plt.plot(model_trial, model_time(model_trials), alpha = .5)
plt.show()
```
...
plt.plot(model_trials, model_time(model_trials), alpha = .5)
plt.show()
```


```{python model_times, echo = F}
...
* a sudoku puzzle with a `top95` or `hardest` difficulty is expected to be logically solved in about `r round(model_time_0, 3)` seconds
* around 90% of sudoku puzzles with a `top95` or `hardest` difficulty is expected to be solved in about 50 attempts (trials), or `r round(model_time_50, 3)` seconds
